...
The bar and the fish are read from the reel track's *structure* rather than
its colour (see :mod:`src.reel_vision`), which is what makes one profile work
across rods that draw the bar white, dark red or as a rainbow gradient. The
per-rod HSV ranges the profile still carries are a leftover of the colour
matcher that came before it: nothing reads them, and the calibration window no
longer asks for them. Shake detection is still colour-based, with a VFX filter
stripping bright flashes and particle effects before analysis.
"""
